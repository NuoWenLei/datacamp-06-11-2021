...
police_nyc <- read_csv("https://raw.githubusercontent.com/Financial-Times/police-misconduct-complaints-analysis/main/output/nyc_clean.csv", col_types=cols()) %>% group_by(officer_id) %>%
  summarize(total=n(), .groups="drop") %>% 
  mutate(chunk_num=ntile(total, 10)) %>% 
  group_by(chunk_num) %>%
  summarize(chunk_total=sum(total), .groups="drop") %>%
...
police_chicago <- read_csv("https://raw.githubusercontent.com/Financial-Times/police-misconduct-complaints-analysis/main/output/chicago_clean.csv", col_types=cols()) %>% group_by(officer_id) %>%
  summarize(total=n(), .groups="drop") %>% 
  mutate(chunk_num=ntile(total, 10)) %>% 
  group_by(chunk_num) %>%
  summarize(chunk_total=sum(total), .groups="drop") %>%
...
ggplot(data=full_data, mapping=aes(x=chunk_num, y=percent_total)) + geom_col() + labs(title="Distribution of Offenses by Decile",
